...
void setupPCA()
{
    ...
    delay(10);
    // Enable auto-increment so multiple registers can be written in one I2C sequence.
...
public:
     ...
     *
     * The command must start with this motor's code and end with a known direction.
     */
     ...
     */
    virtual char stopDirection() const
    {
        return dirs.stop;
    }
 ...
 *
 * Commands are matched against this array in order, so motor codes must be unique.
 */
MotorBase *motors[] = {&motor1, &motor2, &motor3, &motor4, &motor5, &motor6, &motor7, &motor8, &motor9, &motor10, &motor11, &motor12, &motor13};
#pragma endregion
...
void stopAllMotors()
{
    ...
    for (auto motor : motors)
        motor->drive(motor->stopDirection());
}
...
void setup()
{
    ...
    digitalWrite(15, LOW);
    digitalWrite(5, LOW);
    // Hơi vô dụng vì sau đó đã được cấu hình lại trong begin() của MotorGPIO, và tắt trong stopAllMotors()
    // Đề xuất dùng PCA module
